import Link from "next/link";
import Image from "next/image";

export default function Footer() {
    const year = new Date().getFullYear();

    return (
        <footer className="border-t border-gray-200 bg-white">
            <div className="max-w-[1120px] mx-auto px-5">

                {/* Main Footer Content */}
                <div className="py-12 grid grid-cols-1 md:grid-cols-4 gap-10 md:gap-8">

                    {/* Brand Column */}
                    <div className="md:col-span-1">
                        <Link href="/" className="flex items-center gap-2 mb-4">
                            <Image
                                src="/logo-icon.png"
                                alt="Workers United"
                                width={40}
                                height={40}
                                className="h-10 w-10 object-contain"
                            />
                            <span className="font-bold text-[#183b56] text-lg tracking-tight">
                                Workers United
                            </span>
                        </Link>
                        <p className="text-sm text-[#6c7a89] leading-relaxed">
                            International hiring made simple & legal.
                            Connecting serious employers with reliable workers across Europe.
                        </p>
                    </div>

                    {/* Company Column */}
                    <div>
                        <h4 className="font-semibold text-[#183b56] text-sm uppercase tracking-wider mb-4">
                            Company
                        </h4>
                        <ul className="space-y-3">
                            <li>
                                <Link href="/#how-it-works" className="text-sm text-[#6c7a89] hover:text-[#2f6fed] transition-colors">
                                    How it works
                                </Link>
                            </li>
                            <li>
                                <Link href="/#workers" className="text-sm text-[#6c7a89] hover:text-[#2f6fed] transition-colors">
                                    For Workers
                                </Link>
                            </li>
                            <li>
                                <Link href="/#employers" className="text-sm text-[#6c7a89] hover:text-[#2f6fed] transition-colors">
                                    For Employers
                                </Link>
                            </li>
                            <li>
                                <a href="mailto:contact@workersunited.eu" className="text-sm text-[#6c7a89] hover:text-[#2f6fed] transition-colors">
                                    Contact Us
                                </a>
                            </li>
                        </ul>
                    </div>

                    {/* Legal Column */}
                    <div>
                        <h4 className="font-semibold text-[#183b56] text-sm uppercase tracking-wider mb-4">
                            Legal
                        </h4>
                        <ul className="space-y-3">
                            <li>
                                <Link href="/privacy-policy" className="text-sm text-[#6c7a89] hover:text-[#2f6fed] transition-colors">
                                    Privacy Policy
                                </Link>
                            </li>
                            <li>
                                <Link href="/terms" className="text-sm text-[#6c7a89] hover:text-[#2f6fed] transition-colors">
                                    Terms and Conditions
                                </Link>
                            </li>
                        </ul>
                    </div>

                    {/* Connect Column */}
                    <div>
                        <h4 className="font-semibold text-[#183b56] text-sm uppercase tracking-wider mb-4">
                            Connect
                        </h4>
                        <a
                            href="mailto:contact@workersunited.eu"
                            className="text-sm text-[#2f6fed] hover:text-[#1c4dd6] font-medium transition-colors"
                        >
                            contact@workersunited.eu
                        </a>

                        {/* Social Icons */}
                        <div className="flex gap-2 mt-4 flex-wrap">
                            {[
                                {
                                    href: "https://www.facebook.com/profile.php?id=61585104076725",
                                    label: "Facebook",
                                    icon: <path d="M13.5 22v-8h2.7l.4-3h-3.1V9.1c0-.9.3-1.6 1.6-1.6H16.7V4.9c-.3 0-1.4-.1-2.7-.1-2.7 0-4.5 1.6-4.5 4.6V11H7v3h2.8v8h3.7z" />,
                                },
                                {
                                    href: "https://www.instagram.com/workersunited.eu/",
                                    label: "Instagram",
                                    icon: <><path d="M7.5 2h9A5.5 5.5 0 0 1 22 7.5v9A5.5 5.5 0 0 1 16.5 22h-9A5.5 5.5 0 0 1 2 16.5v-9A5.5 5.5 0 0 1 7.5 2zm9 2h-9A3.5 3.5 0 0 0 4 7.5v9A3.5 3.5 0 0 0 7.5 20h9A3.5 3.5 0 0 0 20 16.5v-9A3.5 3.5 0 0 0 16.5 4z" /><path d="M12 7a5 5 0 1 1 0 10 5 5 0 0 1 0-10zm0 2.1A2.9 2.9 0 1 0 12 15a2.9 2.9 0 0 0 0-5.9z" /><path d="M17.6 6.3a1.2 1.2 0 1 1-2.4 0 1.2 1.2 0 0 1 2.4 0z" /></>,
                                },
                                {
                                    href: "https://www.threads.com/@workersunited.eu",
                                    label: "Threads",
                                    isImage: true,
                                },
                                {
                                    href: "https://wa.me/15557839521",
                                    label: "WhatsApp",
                                    icon: <path d="M12.004 0h-.008C5.478 0 0 5.382 0 12.004c0 2.625.846 5.058 2.284 7.034L.79 23.468l4.59-1.468A11.94 11.94 0 0 0 12.004 24C18.621 24 24 18.621 24 12.004 24 5.382 18.621 0 12.004 0zm7 16.956c-.293.825-1.449 1.509-2.373 1.709-.633.135-1.46.241-4.244-.912-3.564-1.476-5.854-5.082-6.03-5.319-.17-.237-1.412-1.881-1.412-3.588s.891-2.547 1.209-2.897c.293-.321.638-.402.85-.402.213 0 .426.002.612.011.197.009.46-.075.72.549.267.642.909 2.216.988 2.376.081.162.135.351.027.563-.107.213-.16.347-.321.534-.16.186-.337.417-.483.56-.16.16-.328.334-.141.657.188.321.833 1.377 1.788 2.231 1.229 1.098 2.262 1.438 2.584 1.599.321.16.51.135.697-.081.188-.218.804-.936 1.018-1.257.214-.321.427-.267.72-.16.293.107 1.863.879 2.184 1.04.321.16.535.241.615.375.081.133.081.771-.212 1.516z" />,
                                    hoverClass: "hover:bg-[#25D366]/10 hover:text-[#25D366]",
                                },
                                {
                                    href: "https://x.com/WorkersUnitedEU",
                                    label: "X",
                                    icon: <path d="M18.7 2H21l-6.7 7.6L22 22h-6.1l-4.8-6.2L5.6 22H3.3l7.2-8.2L2 2h6.2l4.3 5.6L18.7 2zm-1.1 18h1.2L6.3 3.9H5.1L17.6 20z" />,
                                },
                                {
                                    href: "https://www.tiktok.com/@workersunited.eu",
                                    label: "TikTok",
                                    icon: <path d="M14 2h2.2c.2 1.8 1.2 3.2 3.8 3.6V8c-1.7 0-3.2-.6-4.1-1.4V14c0 4-2.7 6-6 6-2.5 0-4.9-1.7-4.9-4.9 0-3.1 2.4-5 5.4-5 .5 0 1 .1 1.5.2V13c-.4-.2-.9-.3-1.5-.3-1.3 0-2.6.8-2.6 2.4 0 1.5 1.1 2.4 2.5 2.4 1.7 0 2.6-1.1 2.6-3V2z" />,
                                },
                                {
                                    href: "https://www.linkedin.com/company/workersunited-eu/",
                                    label: "LinkedIn",
                                    icon: <path d="M4.5 3.5A2 2 0 1 1 4.5 7.5a2 2 0 0 1 0-4zM3 9h3v12H3V9zm7 0h2.9v1.6h.1c.4-.8 1.6-1.7 3.2-1.7 3.4 0 4 2.2 4 5.1V21h-3v-6.1c0-1.5 0-3.3-2-3.3s-2.3 1.6-2.3 3.2V21h-3V9z" />,
                                },
                            ].map((social) => (
                                <a
                                    key={social.label}
                                    href={social.href}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className={`w-9 h-9 flex items-center justify-center rounded-full border border-gray-200 text-[#6c7a89] ${social.hoverClass || "hover:bg-[#f0f4ff] hover:text-[#2f6fed] hover:border-[#2f6fed]/30"} transition-all duration-200`}
                                    aria-label={social.label}
                                >
                                    {social.isImage ? (
                                        <Image src="/threads-logo.svg" alt="Threads" width={16} height={16} className="w-4 h-4 opacity-50" />
                                    ) : (
                                        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current">{social.icon}</svg>
                                    )}
                                </a>
                            ))}
                        </div>
                    </div>
                </div>

                {/* Bottom Bar */}
                <div className="border-t border-gray-100 py-5 flex flex-col sm:flex-row justify-between items-center gap-3">
                    <div className="text-xs text-[#9ca3af]">
                        © {year} Workers United LLC · 75 E 3rd St., Sheridan, WY 82801, USA
                    </div>
                    <div className="flex gap-4 text-xs text-[#9ca3af]">
                        <Link href="/privacy-policy" className="hover:text-[#6c7a89] transition-colors">
                            Privacy
                        </Link>
                        <Link href="/terms" className="hover:text-[#6c7a89] transition-colors">
                            Terms
                        </Link>
                    </div>
                </div>
            </div>
        </footer>
    );
}
