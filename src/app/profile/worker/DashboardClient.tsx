    ...
    return (
        <div className="space-y-6">
            <WorkspaceHero
                ...
                badgeLabel="Worker Workspace"
                title={displayName}
                summary={workspaceSummary}
                meta={[worker?.nationality, worker?.current_country, worker?.preferred_job].filter(Boolean).join(" · ") || "Worker profile"}
                metrics={[
                    { label: "Completion", value: `${profileCompletion}%` },
                    { label: "Documents", value: `${verifiedDocuments}/3` },
                    { label: "Offers", value: activeOfferCount },
