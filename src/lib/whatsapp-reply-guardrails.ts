...
export interface GuardrailPlatformContact {
    websiteUrl?: string | null;
    supportEmail?: string | null;
}

function resolveGuardrailPlatformContact(platform?: GuardrailPlatformContact) {
...
export function getMediaAttachmentResponse(language: string, platform?: GuardrailPlatformContact): string {
...
function getEscalationGuardReply(
    ...
    workerRecord: WhatsAppReplyGuardrailWorkerRecord | null | undefined,
    platform?: GuardrailPlatformContact
): string {
    const { supportEmail } = resolveGuardrailPlatformContact(platform);
    if (language === "sr") {
        return workerRecord?.entry_fee_paid
            ? `Žao mi je zbog problema. Ne mogu da otvorim ili ažuriram interni tehnički ticket direktno sa WhatsApp-a i ne treba da obećavam odgovor tima ovde. Ako ste već platili Job Finder, pošaljite poruku kroz support inbox u dashboard-u ili pošaljite screenshot i kratak opis na ${supportEmail}.`
            : `Žao mi je zbog problema. Ne mogu da otvorim ili ažuriram interni tehnički ticket direktno sa WhatsApp-a i ne treba da obećavam odgovor tima ovde. Pošaljite screenshot i kratak opis problema na ${supportEmail}, zajedno sa svojim brojem telefona, pa će zahtev biti pregledan kroz zvanični kanal.`;
    }
    if (language === "ar") {
        return workerRecord?.entry_fee_paid
            ? `آسف بسبب هذه المشكلة. لا يمكنني فتح أو تحديث تذكرة تقنية داخلية مباشرة من WhatsApp، ولا ينبغي لي أن أعدك برد من الفريق هنا. إذا كنت قد دفعت Job Finder بالفعل، فاستخدم صندوق الدعم داخل لوحة التحكم أو أرسل لقطة الشاشة ووصفًا قصيرًا إلى ${supportEmail}.`
            : `آسف بسبب هذه المشكلة. لا يمكنني فتح أو تحديث تذكرة تقنية داخلية مباشرة من WhatsApp، ولا ينبغي لي أن أعدك برد من الفريق هنا. أرسل لقطة الشاشة ووصفًا قصيرًا للمشكلة إلى ${supportEmail} مع رقم هاتفك ليتم التعامل معها عبر القناة الرسمية.`;
    }
    if (language === "fr") {
        return workerRecord?.entry_fee_paid
            ...
            : `Désolé pour ce problème. Je ne peux pas ouvrir ni mettre à jour un ticket technique interne directement depuis WhatsApp, et je ne dois pas promettre une réponse d’équipe ici. Envoyez la capture d’écran et une courte description du problème à ${supportEmail} avec votre numéro de téléphone pour le suivi officiel.`;
    }
    if (language === "pt") {
        return workerRecord?.entry_fee_paid
            ? `Lamento por esse problema. Eu não posso abrir nem atualizar um ticket técnico interno diretamente pelo WhatsApp, e não devo prometer uma resposta da equipe por aqui. Se o Job Finder já foi pago, use a caixa de suporte no painel ou envie a captura de tela com uma breve descrição para ${supportEmail}.`
            : `Lamento por esse problema. Eu não posso abrir nem atualizar um ticket técnico interno diretamente pelo WhatsApp, e não devo prometer uma resposta da equipe por aqui. Envie a captura de tela e uma breve descrição do problema para ${supportEmail} com seu número de telefone para o atendimento oficial.`;
    }
    if (language === "hi") {
        return workerRecord?.entry_fee_paid
            ? `इस समस्या के लिए खेद है। मैं WhatsApp से सीधे कोई internal technical ticket खोल या अपडेट नहीं कर सकता, और मुझे यहाँ टीम के जवाब का वादा भी नहीं करना चाहिए। अगर आपने Job Finder पहले ही pay कर दिया है, तो dashboard support inbox का उपयोग करें या screenshot और short description ${supportEmail} पर भेजें।`
            : `इस समस्या के लिए खेद है। मैं WhatsApp से सीधे कोई internal technical ticket खोल या अपडेट नहीं कर सकता, और मुझे यहाँ टीम के जवाब का वादा भी नहीं करना चाहिए। कृपया screenshot, short description और अपना phone number ${supportEmail} पर भेजें ताकि इसे official channel से देखा जा सके।`;
    }

    return workerRecord?.entry_fee_paid
        ? `Sorry this is still causing trouble. I can’t open or update an internal technical ticket directly from WhatsApp, and I shouldn’t promise a team reply here. If you already paid for Job Finder, please use the support inbox in your dashboard or send the screenshot and a short description to ${supportEmail}.`
        : `Sorry this is still causing trouble. I can’t open or update an internal technical ticket directly from WhatsApp, and I shouldn’t promise a team reply here. Please send the screenshot, a short description, and your phone number to ${supportEmail} so it can be reviewed through the official channel.`;
}
...
function getPaymentGuardReply(
    ...
    if (workerRecord.entry_fee_paid) {
        if (language === "sr") {
            return `Vaša Job Finder uplata je već evidentirana, tako da nema novog payment linka za slanje ovde. Sledeći korak i status pratite iz dashboard-a na ${workerProfileUrl}.`;
        }
        if (language === "ar") {
            return `تم تسجيل دفعة Job Finder بالفعل، لذلك لا يوجد رابط دفع جديد لإرساله هنا. تابع الحالة والخطوة التالية من لوحة التحكم على ${workerProfileUrl}.`;
        }
        if (language === "fr") {
            return `Votre paiement Job Finder est déjà enregistré, donc il n’y a pas de nouveau lien de paiement à envoyer ici. Suivez le statut et la prochaine étape depuis le tableau de bord sur ${workerProfileUrl}.`;
        }
        if (language === "pt") {
            return `Seu pagamento do Job Finder já está registrado, então não há novo link de pagamento para enviar aqui. Acompanhe o status e a próxima etapa no painel em ${workerProfileUrl}.`;
        }
        if (language === "hi") {
            return `आपका Job Finder payment पहले से दर्ज है, इसलिए यहाँ कोई नया payment link भेजने की ज़रूरत नहीं है। अगला step और status ${workerProfileUrl} dashboard में देखें।`;
        }
        return `Your Job Finder payment is already recorded, so there is no new payment link to send here. Please follow your status and next step in the dashboard at ${workerProfileUrl}.`;
    }
    ...
    if (language === "sr") {
        return `Vaš Job Finder checkout je otključan, ali uplata se i dalje pokreće samo iz dashboard-a. Otvorite ${workerProfileUrl} i tamo započnite bezbedan checkout; payment link ne šaljemo preko WhatsApp-a.`;
    }
    if (language === "ar") {
        return `تم فتح Checkout الخاص بـ Job Finder لحسابك، لكن الدفع يبدأ فقط من لوحة التحكم. افتح ${workerProfileUrl} وابدأ الدفع الآمن من هناك؛ نحن لا نرسل روابط الدفع عبر WhatsApp.`;
    }
    if (language === "fr") {
        return `Le checkout Job Finder est débloqué pour votre compte, mais le paiement démarre uniquement depuis le tableau de bord. Ouvrez ${workerProfileUrl} et lancez le checkout sécurisé depuis là; nous n’envoyons pas de lien de paiement via WhatsApp.`;
    }
    if (language === "pt") {
        return `O checkout do Job Finder está liberado para sua conta, mas o pagamento começa apenas pelo painel. Abra ${workerProfileUrl} e inicie o checkout seguro por lá; não enviamos link de pagamento pelo WhatsApp.`;
    }
    if (language === "hi") {
        return `आपके account के लिए Job Finder checkout unlock है, लेकिन payment फिर भी सिर्फ dashboard से शुरू होता है। ${workerProfileUrl} खोलिए और वहीं से secure checkout शुरू कीजिए; हम WhatsApp पर payment link नहीं भेजते।`;
    }
    return `Job Finder checkout is unlocked for your account, but payment still starts only from the dashboard. Open ${workerProfileUrl} and start the secure checkout there; we do not send payment links over WhatsApp.`;
}
...
function getAvailabilityGuardReply(
    ...
    const { signupUrl } = resolveGuardrailPlatformContact(platform);
    if (language === "sr") {
        return workerRecord
            ...
            : `Ne mogu da potvrdim konkretan otvoren posao preko WhatsApp-a. Workers United radi kroz vođeni matching proces; prvo napravite nalog na ${signupUrl} i popunite profil, a zatim nastavljamo kroz dashboard i pitanja o procesu.`;
    }
    if (language === "ar") {
        return workerRecord
            ...
            : `لا أستطيع تأكيد وظيفة محددة مفتوحة عبر WhatsApp. يعمل Workers United من خلال مطابقة موجهة؛ أنشئ حسابك أولاً على ${signupUrl} وأكمل ملفك، ثم نتابع من خلال لوحة التحكم وأسئلة العملية.`;
    }
    if (language === "fr") {
        return workerRecord
            ...
            : `Je ne peux pas confirmer une offre précise ouverte via WhatsApp. Workers United fonctionne avec un processus d’appariement guidé ; créez d’abord votre compte sur ${signupUrl} et complétez votre profil, puis nous continuons via le tableau de bord et les questions sur le processus.`;
    }
    if (language === "pt") {
        return workerRecord
            ...
            : `Eu não posso confirmar uma vaga específica aberta pelo WhatsApp. A Workers United trabalha com um processo guiado de matching; primeiro crie sua conta em ${signupUrl} e complete seu perfil, e depois seguimos pelo painel e pelas dúvidas sobre o processo.`;
    }
    if (language === "hi") {
        return workerRecord
            ...
            : `मैं WhatsApp पर किसी specific open job की पुष्टि नहीं कर सकता। Workers United guided matching process के ज़रिए काम करता है; पहले ${signupUrl} पर account बनाइए और profile पूरा कीजिए, फिर dashboard और process सवालों के साथ आगे बढ़ते हैं।`;
    }
    ...
    return workerRecord
        ...
        : `I cannot confirm a specific open job over WhatsApp. Workers United works through a guided matching process; first create your account at ${signupUrl} and complete your profile, then we continue through the dashboard and any process questions here.`;
}
...
export function applyWhatsAppReplyGuardrails({
    ...
    if (GUARDED_ESCALATION_PATTERNS.some((pattern) => pattern.test(trimmed))) {
        return {
            text: getEscalationGuardReply(guardrailLanguage, workerRecord, platform),
            triggered: true,
            reason: "escalation",
        };
    }
    ...
    if (GUARDED_PAYMENT_PATTERNS.some((pattern) => pattern.test(trimmed))) {
        return {
            text: getPaymentGuardReply(guardrailLanguage, workerRecord, platform),
            triggered: true,
            reason: "payment",
        };
    }
    ...
    if (GUARDED_AVAILABILITY_PATTERNS.some((pattern) => pattern.test(trimmed))) {
        return {
            text: getAvailabilityGuardReply(guardrailLanguage, workerRecord, platform),
            triggered: true,
            reason: null,
        };
    }
