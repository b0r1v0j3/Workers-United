...
### ✅ Završeno (poslednje)
- [x] Agency worker operations pass: `/profile/agency` tabela sada prikazuje `#` i `Created` po workeru, svaki claimed a neplaćeni worker ima direktan `$9` Job Finder checkout CTA, draft + claimed worker nalozi mogu da se brišu i pojedinačno i kroz multi-select bulk delete, a bulk payment je namerno zaključan jer trenutni Stripe/webhook model bezbedno podržava samo jedan payment target po checkout-u — 08.03.2026
