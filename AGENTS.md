...
### ✅ Završeno (poslednje)
- [x] Stripe finalize source-of-truth pass: novi shared `src/lib/stripe-payment-finalization.ts` sada drži kanonske Stripe amount-e, completed checkout payment upsert i entry-fee worker queue activation za direct i agency worker tok, pa `src/app/api/stripe/webhook/route.ts` i `src/app/api/stripe/confirm-session/route.ts` više ne održavaju dve verzije iste post-payment logike; dodat je regression test `__tests__/stripe-payment-finalization.test.ts`, a confirm-session zadržava samo svoj response/email fallback sloj dok webhook zadržava telemetry/logging side effecte — 19.03.2026
