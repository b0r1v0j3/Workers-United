...
> **Poslednje ažuriranje:** 14.03.2026 (Worker `candidate -> worker` cutover je završen end-to-end: live public schema više ne izlaže `candidates` / `candidate_documents`, `contract_data` koristi samo `worker_*` override kolone, jedini preostali storage bucket je `worker-docs`, auth signup trigger sada upisuje direktno u kanonski `workers`, worker workspace shell je vizuelno poravnat, worker overview kartice sada prate isti blaži radius ritam kao sidebar, sidebar logout je zakucan za dno bez mrtvog prostora, logout confirm se renderuje kroz pravi portal na sredini ekrana umesto unutar sidebara, agency sidebar više ne duplira isti `/profile/agency` ulaz kroz `Overview + Agency Workers`, agency worker tabela više ne beži na poseban `Open` ekran nego koristi isti modal/form za `Add + Edit worker`, mobile workspace shell je dodatno zategnut sa užim rail-om, manjim left offset-om, nižim agency hero-jem i manjim ujednačenim shell/card radiusom, a desktop shell sada ima levi gutter umesto lepljenja za ivicu ekrana i uži collapsed rail da više liči na mobilni; agency lista sada ima redni broj, trajni `Added` datum preko novog immutable `workers.created_at` signala, single-worker `$9` checkout CTA, row/bulk delete za draft i claimed worker naloge, bulk payment ostaje zaključan dok Stripe/webhook ne podrže siguran multi-worker total, agency sada može direktno da plati entry fee i za draft worker-e bez `claim link / claim first` koraka, a otvoreni ali neplaćeni Stripe checkout više se ne izlaže agency-u kao posebna završna faza: worker ostaje `Not paid yet / Pay $9`, dok isto dugme po potrebi tiho nastavlja postojeći checkout session; `Status` kolona više ne duplira `Completion` nego prikazuje stvarnu operativnu fazu (`Not paid yet`, `Paid`, `Waiting X days`, `Job offered`, `Offer accepted`, `Visa`, `Refund`, `Placed`) na osnovu payment/refund/queue signala; agency worker board je vraćen na klasičnu tabelu sa jačim horizontalnim i vertikalnim separatorima po kolonama, worker ime na desktopu sada lomi prezime u drugi red da tabela stane bez horizontalnog scroll-a, a `Completion` polje sada ima pravi progress meter/skalu umesto golog procenta; agency support sada ima sopstveni `/profile/agency/inbox` ulaz u shared shell-u, koristi isti support thread backend kao worker inbox, ostaje uvek otključan za agency bez zavisnosti od `$9` uplate, a i worker i agency inbox sada ponovo renderuju isti neutralni workspace shell bez starog standalone/beige layout-a; Brain/ops monitoring takođe ispravno parsira dnevni report, šalje exception mail, razlikuje platform-side WhatsApp template kvar od nedostavljivog broja/zemlje i `system-smoke` više ne proglašava opcioni degradirani servis potpuno zdravim, dok je `profile_incomplete` WhatsApp tok dodatno očvršćen tako da više ne pokušava zastareli CTA-button shape pre slanja nego odmah koristi kanonski body-only template payload koji prolazi u live Meta varijanti, Brain pricing memory sada ima guard protiv lažnih `employer pays placement fee` tvrdnji, auth email/magic-link/recovery hash flow sada završava session kroz shared post-auth resolver + `/api/auth/finalize`, `/login` ume da dovrši Supabase hash link i vrati korisnika u pravi workspace, ključni worker/employer/agency/signup mobilni flow-ovi prolaze bez horizontalnog skrola na 320px i 390px, mobilni workspace header sada koristi hamburger bez wordmark-a uz čist full-height drawer bez spoljnog rounded-shell okvira, veliki spoljnji worker/support/inbox section card-ovi su spljošteni na telefonu i umesto njih koriste tanke uvučene separator linije koje ne dodiruju ivice ekrana, a header action dugmad sada su uparena: hamburger koristi isti round control + separator tretman kao notifications, notifications se otvaraju kao desni sidebar drawer koji zatvara levi meni pre otvaranja, isti desni drawer sada koristi bukvalno isti `transition-all + transform + opacity` model kao levi sidebar tako da klizi sa desna na levo umesto da samo fade-uje pri otvaranju, shared mobile workspace gutter je dodatno pojačan kroz `AppShell` i flat worker/support sekcije tako da sadržaj više ne deluje nalepljeno uz levu ivicu na telefonu, a root uzrok preostale asimetrije je uklonjen iz `AppShell`: mobilni `mainOffsetClass` više ne nameće `pl-0`, pa levi i desni workspace inset konačno koriste isti osnovni shell padding, dok novi workspace-wide mobile spacing pass sada drži admin sandbox banner, worker edit/documents/queue, employer overview/post-job i agency overview/add-worker modal na istom simetričnom inset i flat-section ritmu, a agency mobilni `Add worker` flow sada više ne pokušava skučeni popup nego otvara punu `/profile/agency/workers/new` stranicu, dok desktop zadržava modal.)
