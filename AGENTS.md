...
- **Navbar dizajn:** `UnifiedNavbar.tsx` koristi čist public header (`h-[80px] sm:h-[92px] md:h-[108px]`) sa stalnim glass efektom (`bg-white/95 backdrop-blur-md`), bez srednjih public linkova (`How it works / For Workers / For Employers`) i bez mobile hamburger menija. Public variant nema border/shadow liniju, ostaje sticky pri skrolu, a sadržaj je full-width (`w-full` + responsive horizontal padding) sa logo blokom skroz levo i akcijama skroz desno. Dinamički scroll (bubrenje/skupljanje) je **ukinut**. 
