...
- [x] Agency draft documents + review parity: agency sada može da uploaduje/verifikuje/re-queue document review i za draft worker profile bez claim-a; `src/app/api/agency/workers/[workerId]/documents/route.ts`, `src/app/api/documents/request-review/route.ts`, `src/app/api/verify-document/route.ts`, `src/lib/agencies.ts`, `src/app/profile/agency/page.tsx` i `src/app/profile/agency/workers/[id]/page.tsx` dosledno koriste `profile_id || worker.id` kao document owner, dok dashboard/detail sada daju pravi `Upload docs` ulaz direktno iz `Documents` kolone i detail sekcije — 14.03.2026
