...
### ✅ Završeno (poslednje)
- [x] Shared job-preferences picker pass: agency `Add/Edit worker` i worker `Edit profile` sada dele novi `src/components/forms/PreferenceSheetField.tsx` sheet/popup picker za `Preferred job` i `Preferred destinations`, `Select industry` placeholder je zamenjen vidljivim `All industries` izborom, destinations više ne koriste stari checkbox grid, a agency dashboard prikaz prevodi legacy `Any` u čitljiv `All industries` bez promene kompatibilnosti backend `Any` signala — 14.03.2026
