...
| `src/lib/whatsapp-brain.ts` | Shared canonical WhatsApp facts/rules, safer worker/employer prompting, deterministic first-contact greeting reply, conversation-aware language resolution for short/colloquial follow-ups, explicit `profile complete + admin approval -> payment unlock` guard language, no-fake-escalation rule set, onboarding trigger detection, and low-risk learning filter used by `/api/whatsapp/webhook` + `/api/brain/improve` |
...
| `src/lib/whatsapp-conversation-helpers.ts` | Shared WhatsApp conversation helpers; loads recent `whatsapp_messages`, filters safe `brain_memory`, formats transcripts for prompts, and creates worker support auto-handoff summaries/flags so `src/app/api/whatsapp/webhook/route.ts` does not hardcode those side-effect helpers inline |
...
| `src/lib/whatsapp-fallback.ts` | Shared config-driven WhatsApp fallback helper; loads platform config and serves multilingual signup/status/payment/documents fallback copy so the webhook route no longer hardcodes the fallback bot block inline, while preserving Hindi as Hindi instead of the old Nepali fallback drift |
| `src/lib/whatsapp-identity.ts` | Shared WhatsApp identity helper; resolves worker/profile context from direct worker phone matches plus auth-phone fallback/backfill so the webhook route no longer hardcodes that multi-layer lookup inline |
