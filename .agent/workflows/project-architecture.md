...
| `src/lib/whatsapp-inbound-events.ts` | Shared inbound WhatsApp helper; normalizes phone numbers, extracts text/media content, atomically records inbound message rows with duplicate-safe `wamid` handling, and can attach `user_id` after identity resolution so the webhook route no longer hardcodes normalize/dedupe/log plumbing inline |
