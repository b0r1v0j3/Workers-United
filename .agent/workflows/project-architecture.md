...
│   │   │   ├── admin/         # delete-user, employer-status, funnel-metrics (now including payment-quality breakdown plus worker/billing-country issue signals), admin inbox support list, agency-worker approval API with truthful approval-email notification payloads, authenticated `send-campaign` outreach dispatch via `queueEmail()`, authenticated WhatsApp blast dispatch via the shared blast helper/strict payment-readiness gate, and same-origin document preview streaming with legacy image auto-rotation self-heal; manual-match/re-verify are now fully workerId-first
