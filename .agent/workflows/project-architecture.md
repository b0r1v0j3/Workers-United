...
| Database | **Supabase (PostgreSQL)** | RLS policies, cron-triggered functions, in-platform messaging tables (`conversations*`); worker app-layer runtime reads/writes through `worker_onboarding` / `worker_documents`, live Supabase physically uses `workers` / `worker_documents`, `documents / matches / offers` carry only canonical `worker_id` FKs, `contract_data` worker overrides are `worker_*`, the live public schema no longer exposes the old `candidates` / `candidate_documents` aliases, and canonical employer identity is now limited to `tax_id (PIB)` + `company_registration_number / mb (maticni broj)` without the retired `business_registry_number / employer_apr_number` layer |
...
│   ├── migrations/            # Incremental migrations (including live worker-first cutover stages, pending employer dedupe, `20260404123000_admin_whatsapp_thread_views.sql` for admin WhatsApp normalized thread/message views, `20260404150000_admin_whatsapp_thread_views_state.sql` for per-admin unread/seen state, and `20260406223000_drop_employer_registry_legacy_fields.sql` which removes the retired employer APR/business-registry fields from live schema)
│   └── ...                    # Other SQL patches
├── src/
│   ├── app/
...
│   │   ├── admin/
│   │   │   ├── page.tsx       # Business admin dashboard (stats, action cards, queue watch, inbox, recent lists, direct `Preview Worker/Employer/Agency` entry points, direct `WhatsApp` / `Email Preview` admin shortcuts, and inspect links into real workspaces); preview cards are generic read-only entries, not derived from the admin's own legacy role rows
...
│   │   │   ├── whatsapp/      # Admin WhatsApp console: thread list, phone/profile linking, transcript detail, direct jumps into worker case/workspace when known, DB-backed thread summaries, lazy transcript loading for the selected phone, paged/problem-filter drill-down, saved ops views for triage, and per-admin unread/seen badges driven by explicit thread opens
...
│   │   │   ├── email-queue/   # Email queue inspection/insert API; shared retry processor lives in `src/lib/email-queue.ts`
...
│   │   ├── admin/AdminWhatsAppSeenTracker.tsx # Client-side seen-state tracker for `/admin/whatsapp`; only marks explicitly opened inbound threads as seen and refreshes the server view after a successful write
...
│   │   ├── admin/DocumentPreview.tsx # Admin contract-payload preview card aligned with the worker case ops UI; employer payload preview now shows only canonical company identifiers (`PIB`, `MB`) and no longer surfaces the retired APR/business-registry field
...
│   │   ├── platform-contact.ts # Client-safe canonical public URL/support-email/public-WhatsApp normalization used by platform-config, homepage/schema/footer/floating CTA, email templates, WhatsApp CTA suffix handling, worker approval payloads, signout/legal/auth/settings surfaces, and admin email-preview mocks
│   │   ├── cron-auth.ts # Shared CRON_SECRET bearer-auth helper used by cron/admin health routes so missing env never degrades into a fail-open `Bearer undefined` check
...
│   │   ├── offer-checkout-copy.ts # Shared worker offer/checkout truthfulness copy so offer CTAs no longer hardcode a fixed placement amount or drift away from secure-checkout wording
...
│   │   ├── stripe-checkout-pending.ts # Shared reusable pending checkout lookup + session-create failure cleanup helpers
...
│   │   ├── queue-payment-return.ts # Shared worker-queue payment-return mode helper so `payment=success` without `session_id` stays in verification-pending copy instead of firing a false success toast/confetti path
...
│   │   ├── ops-monitor.ts     # Deterministic ops-first daily monitor builder + compact alert email renderer (route health, WhatsApp, docs, email, payments, auth), now including outbound WhatsApp message-log durability failures
...
│   │   ├── whatsapp.ts        # WhatsApp Cloud API (template sending, text sending, logging, failed-send error capture); proactive template sends now short-circuit when the same number recently hit a recipient-side Meta block/undeliverable error, thrown/network template failures now propagate the same retryable/failure-category metadata as the rest of the send stack, and failed DB log writes emit `whatsapp_message_log_failed` activity for ops visibility
...
│   │   ├── whatsapp-fallback.ts # Shared config-driven multilingual fallback copy extracted from the webhook route, now honoring explicit language-switch requests, recent conversation language, shared multilingual intent hints, and localized public status labels instead of raw worker enums
...
| `src/lib/auth-redirect.ts` | Shared post-auth provisioning/redirect engine used by both `/auth/callback` and `/api/auth/finalize` so admin/employer/agency/worker routing stays consistent, while automated welcome sends are skipped for hidden/internal/test contacts and share the same `pending + sent` email dedupe plus prior WhatsApp `welcome_registration` dedupe; direct post-auth WhatsApp welcome markers are now written only after a real successful send instead of on any best-effort attempt |
...
| File | Role |
|---|---|
| `src/app/admin/exceptions/page.tsx` | Legacy internal-ops route; still renders the shared technical exception cockpit, but the business admin shell no longer links to it and owners should enter through `/internal/ops`; the WhatsApp section now shows both recent auto-handoffs and recent reply-delivery failures with retryable/platform classification |
...
| `src/app/admin/workers/[id]/page.tsx` | Admin worker case view; now loads the canonical worker record, renders same-origin document preview URLs, reads canonical document review fields (`ocr_json`, `reject_reason`, `verified_at`) instead of removed legacy columns, passes image docs into the manual crop-capable review modal, and reuses the shared worker-approval guard/action helper for approve/revoke plus strict status gating |
...
| `src/app/api/admin/documents/[documentId]/preview/route.ts` | Same-origin admin document preview stream; downloads from `worker-docs` with service-role access, returns inline PDF/image responses so browser viewers work inside the admin modal, now runs one-shot AI rotation + auto-crop self-heal for legacy image docs, skips canonical rewrite if backup preservation fails, and its `POST` path both saves manual admin crop selections after first preserving an `_admin-originals/` backup and removes that original backup again on restore |
...
| `src/app/profile/agency/workers/[id]/AgencyWorkerClient.tsx` | Full worker editor for agency-owned workers, including documents, review requests, and Job Finder payment for claimed workers; admin preview now reflects the real approval bit and strict verified-doc completion signal rather than inferring readiness from raw uploaded documents |
...
| `src/lib/worker-review.ts` | Shared worker review/readiness helpers; keeps `PENDING_APPROVAL` gated behind truly admin-verified required documents, safely backfills blank passport issue/expiry/issuer fields from the latest passport `ocr_json` before recalculating completion, now exposes shared `loadWorkerApprovalGuardState()` + `applyWorkerApprovalAction()` helpers so admin and agency approval flows reuse the exact same strict completion/revoke/email rules, blocks approval revocation once Job Finder is active, and only suppresses `profile_complete` retries when the queue row is truly `sent` |
| `src/lib/worker-status.ts` | Shared worker lifecycle/status helper; treats queue/offer/visa/refund states as system-managed after Job Finder activation, restricts manual admin overrides to unpaid pre-payment states, blocks manual `APPROVED`, and keeps the worker detail admin page aligned with the same canonical dropdown/validation rules |
| `src/lib/worker-workspace-state.ts` | Shared worker workspace stage helper; distinguishes queue-active, payment-pending-activation, and advanced post-payment case states so the worker dashboard does not mislabel offer/visa/refund cases as “activating your queue status” |
| `src/lib/admin-worker-notifications.ts` | Shared admin worker page notification helper; centralizes `document / approval / manual-status` banner copy, manual status email payload wording, and `sent / queued / failed` delivery classification so the worker case detail stays truthful without duplicating inline banner logic |
| `src/lib/manual-match-feedback.ts` | Shared manual-match operator feedback helper; turns `/api/admin/manual-match` partial-rollback payloads into admin-safe UI/toast copy so rollback failures no longer disappear behind generic match errors |
...
| `src/lib/cron-auth.ts` | Shared cron/admin bearer-auth helper; centralizes `CRON_SECRET` parsing plus exact bearer validation so cron, email-queue, and detailed health routes fail closed when the env is missing instead of accidentally accepting `Bearer undefined` |
| `src/lib/worker-notification-eligibility.ts` | Shared guard for worker direct notifications; hidden draft-owner auth users never receive welcome/reminder/profile-complete sends, exposes hidden-draft detection as a reusable helper, and agency draft workers only become directly contactable after the agency provides the real worker email plus phone |
...
| `src/lib/offer-checkout-copy.ts` | Shared worker offer/checkout copy helper; keeps the queue card and offer detail screen on the same generic secure-checkout wording without hardcoding a single-country placement amount into the public worker UI |
...
| `src/lib/admin-whatsapp.ts` | Shared admin WhatsApp console helper; prefers DB-backed `admin_whatsapp_thread_summaries` / `admin_whatsapp_message_log` views for normalized thread summaries and lazy transcript loading, resolves worker/employer/agency/admin ownership from profile + phone signals, exposes reusable risk/recent/unread triage helpers for saved views, merges per-admin seen-state from `admin_whatsapp_thread_views`, and falls back to legacy full-table JS grouping if the views are not deployed yet |
| `supabase/migrations/20260404123000_admin_whatsapp_thread_views.sql` | Adds `admin_whatsapp_thread_summaries` and `admin_whatsapp_message_log` views plus a normalized-phone expression index so `/admin/whatsapp` can list thread summaries without loading every raw `whatsapp_messages` row on each request |
| `supabase/migrations/20260404150000_admin_whatsapp_thread_views_state.sql` | Adds `admin_whatsapp_thread_views` for per-admin `last_seen_at` persistence, so unread/seen state on `/admin/whatsapp` survives refreshes and remains isolated per admin profile |
| `src/lib/whatsapp-conversation-helpers.ts` | Shared WhatsApp conversation helpers; loads recent `whatsapp_messages` while filtering failed outbound rows and proactive/template sends from prompt history, filters safe `brain_memory`, formats transcripts for prompts, creates worker support auto-handoff summaries/flags, and escalates repeated retryable reply-delivery failures into support flags so `src/app/api/whatsapp/webhook/route.ts` does not hardcode those side-effect helpers inline |
...
| `src/lib/whatsapp-fallback.ts` | Shared config-driven WhatsApp fallback helper; loads platform config and serves multilingual signup/status/payment/documents fallback copy so the webhook route no longer hardcodes the fallback bot block inline, preserves Hindi as Hindi instead of the old Nepali fallback drift, now covers French/Portuguese payment/status/document branches instead of collapsing them to English, reuses shared multilingual intent hints, localizes public-facing worker status text instead of leaking raw enums, and honors explicit language-switch requests plus recent conversation language during fallback recovery |
...
| `src/lib/whatsapp-status-events.ts` | Shared Meta delivery-status persistence helper; stores `sent/delivered/failed` plus failure details without keeping status merge logic inline in the webhook route, and now throws on DB errors or unmatched `wamid` updates so silent delivery-status durability gaps surface immediately |
...
| `src/lib/whatsapp.ts` | WhatsApp Cloud API — template sending, text sending, logging; failed sends now persist `error_message` into `whatsapp_messages`, proactive template sends suppress recent recipient-side undeliverables, thrown/network template failures preserve `retryable + failureCategory` metadata, recent recipient-block freshness can be reused by reminder automation, and failed DB logging now degrades safely into ops activity instead of flipping real provider-send outcomes |
